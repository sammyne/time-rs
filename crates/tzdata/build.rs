...
fn main() -> anyhow::Result<()> {
    ...
    for p in walk_dir(&zoneinfo_dir).context("walk dir")? {
        ...
        zw.start_file(&name, opts).with_context(|| format!("append {name}"))?;
        zw.write_all(&data).with_context(|| format!("write data for {name}"))?;
...
fn walk_dir(p: &Path) -> anyhow::Result<Vec<PathBuf>> {
    let mut out = vec![];
    ...
    for v in fs::read_dir(p).context("read dir")? {
        ...
        if t.is_file() {
            ...
            let others = walk_dir(&e.path()).with_context(|| format!("walk into dir {:?}", e.path()))?;
            out.extend(others);
        } else {
